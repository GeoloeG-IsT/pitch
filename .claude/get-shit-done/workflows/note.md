...
- **Global scope**: `/home/pascal/wks/pitch/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/pascal/wks/pitch/.claude/notes/`)
...
2. Glob `/home/pascal/wks/pitch/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/pascal/wks/pitch/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/pascal/wks/pitch/.claude/notes/` — works in any directory
